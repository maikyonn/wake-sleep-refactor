...
class MidiDataModule(pl.LightningDataModule):
    def __init__(self, cfg: DataCfg, tokenizer: MusicTokenizerWithStyle,
                 batch_size: int = 32, num_workers: int = 4, drop_last: bool = True, seed: int = 42):
        super().__init__()
        self.cfg = cfg
        self.tok = tokenizer
        ...
        self.seed = seed
        self.save_hyperparameters('batch_size', 'num_workers', 'drop_last', 'seed')

        self._train_dataset: Optional[OnDemandMidiDataset] = None
        self._val_dataset: Optional[OnDemandMidiDataset] = None
        self._test_dataset: Optional[OnDemandMidiDataset] = None # Placeholder for test

    def setup(self, stage: Optional[str] = None):
        # This method is called by PyTorch Lightning.
        # It's called on every GPU in DDP.
        print("MidiDataModule.setup() called for stage: {stage} on Global Rank 0.")
        is_global_zero = not (torch.distributed.is_available() and torch.distributed.is_initialized()) or \
                         (torch.distributed.is_available() and torch.distributed.is_initialized() and torch.distributed.get_rank() == 0)

        if is_global_zero:
            logger.info(f"MidiDataModule.setup() called for stage: {stage} on Global Rank 0.")
        
        # Create train dataset
        self._train_dataset = OnDemandMidiDataset(self.cfg, self.tok, self.cfg.data_dir, is_validation_set=False)
        # Apply shuffling to the record list if configured (for map-style dataset)
        if self.cfg.shuffle_records and self._train_dataset._records: # Check _records exists
            logger.info(f"Shuffling training records for {self.cfg.data_dir}.")
            random.Random(self.seed).shuffle(self._train_dataset._records) # Shuffle in-place

        # Create validation dataset
        if self.cfg.val_data_dir:
            self._val_dataset = OnDemandMidiDataset(self.cfg, self.tok, self.cfg.val_data_dir, is_validation_set=True)
            # Val records are typically not shuffled, but if needed:
            # if self.cfg.shuffle_records_val and self._val_dataset._records: # Add shuffle_records_val to DataCfg
            #    random.Random(self.seed).shuffle(self._val_dataset._records)
        else:
            self._val_dataset = None
            if is_global_zero: logger.info("No validation data directory provided.")
        
        # Test dataset placeholder (can be similar to validation)
        if self.cfg.val_data_dir: # Example: use val data as test if no specific test path
            self._test_dataset = OnDemandMidiDataset(self.cfg, self.tok, self.cfg.val_data_dir, is_validation_set=True)

    def _get_dataloader(self, ds_type: str):
        """
        Return a plain torch.DataLoader.
        Lightning will wrap it in DistributedSampler
        when running under DDP, so we do *not* touch
        sampler / rank logic here.
        """
        # -------- pick the right dataset --------
        dataset, is_train = None, False
        if ds_type == "train":
            dataset, is_train = self._train_dataset, True
        elif ds_type == "val":
            dataset = self._val_dataset
        elif ds_type == "test":
            dataset = self._test_dataset
        else:
            raise ValueError(f"Unknown dataloader type: {ds_type}")
        ...
        return DataLoader(
            ...
            shuffle=is_train and self.cfg.shuffle_records,  # Lightning needs shuffle=True to add its sampler
            num_workers=self.num_workers,
            collate_fn=lambda b: midi_collate_mapstyle(b, self.tok),
        )

    def train_dataloader(self): return self._get_dataloader('train')
    def val_dataloader(self):   return self._get_dataloader('val')
    def test_dataloader(self):  return self._get_dataloader('test')
